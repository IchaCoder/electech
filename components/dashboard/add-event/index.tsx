...
import EventCreatedModal from "./modal";
...
const AddEvent = (props: Props) => {
  ...
  const onSubmit = async (data: FormValueTypes) => {
    ...
    const { message, status, token: id } = await AddEventRequest(data as unknown as IEvent, token!);
    setEventId(id!);
    toast({
      title: message,
      status: status,
  ...
  return (
    <>
      <EventCreatedModal event_id={event_id} isOpen={isOpen} onClose={onClose} />
